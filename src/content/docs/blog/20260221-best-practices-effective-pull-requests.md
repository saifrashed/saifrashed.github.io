...
Ultimately, pull requests have changed code review drastically, but its success still heavily depends on well-written descriptions. They serve as a communication tool to set expectations between the reviewer and the reviewee. While mundane administrative tasks are 'easy wins' for developers, the real value in a description lies in explaining how and why a change is made. It takes more time and effort to write a good description, but investing the time to explain the motivation, trade-offs and effect of the code leads to shorter review cycles and fewer revisions.